...
def writeRedirectsToFile(
    pregenerated_redirects: dict, redirects: list, key: str, redirects_file: str
) -> bool:
    if redirects:
        try:
    ...
    for key in keys:
        if key.find("html") != -1:
            try:
                ...
                print(
                    'Will not find new redirects, exiting "find_redirects" function...'
                )
                return
            if response and "ResponseMetadata" in response:
                ...
                if "HTTPHeaders" in metadata:
                    ...
                    if "x-amz-website-redirect-location" in headers:
                        redirects.append(
                            (key, headers["x-amz-website-redirect-location"])
                        )
    print(
...
def main() -> None:
    ...
    if redirects and len(redirects):
